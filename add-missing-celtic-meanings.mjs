...
async function addMissingMeanings() {
  console.log('Adding missing Celtic meanings...\n');

  for (const [cardName, meanings] of Object.entries(celticMeanings)) {
    ...
    if (!existingCard) {
      console.error(`  ${cardName} not found in database`);
      continue;
    }
    ...
    if (updateError) {
      console.error(`  Error updating ${cardName}:`, updateError);
    } else {
      console.log(`  ✓ ${cardName} updated successfully`);
    }
  }
  ...
  const { data: cards, error } = await supabase
    ...
    .in('name', Object.keys(celticMeanings));
  ...
  const stillMissing = cards.filter(card =>
    !card.celtic_meaning_upright || !card.celtic_meaning_reversed
  );

  if (stillMissing.length > 0) {
    console.log('\nStill missing meanings:');
    stillMissing.forEach(card => console.log(`  - ${card.name}`));
  } else {
    console.log('\n✓ All cards now have Celtic meanings!');
  }
}
