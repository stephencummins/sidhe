-- Safe, idempotent migration for Stripe tables
...
-- Create unique index (drop first if exists to avoid errors)
DROP INDEX IF EXISTS idx_celtic_cross_credits_user_id;
...
-- Add indexes for faster lookups (drop first if exists)
...
DROP INDEX IF EXISTS idx_stripe_purchases_user_id;
DROP INDEX IF EXISTS idx_stripe_purchases_email;
DROP INDEX IF EXISTS idx_stripe_purchases_stripe_session_id;
