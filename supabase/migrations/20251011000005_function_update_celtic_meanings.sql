/*
  # Create Function to Update Celtic Meanings

  Creates a database function that can update card Celtic meanings, bypassing RLS.
  This is needed because the cards are owned by a specific user and the anon key
  cannot directly update them.

  1. New Functions
    - `update_card_celtic_meanings` - Updates Celtic meanings for a specific card
*/

-- Create function to update Celtic meanings
CREATE OR REPLACE FUNCTION update_card_celtic_meanings(
  p_card_name text,
  p_upright text,
  p_reversed text,
  p_keywords text DEFAULT NULL,
  p_mythology text DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  UPDATE tarot_cards
  SET
    celtic_meaning_upright = p_upright,
    celtic_meaning_reversed = p_reversed,
    celtic_keywords = COALESCE(p_keywords, celtic_keywords),
    celtic_mythology = COALESCE(p_mythology, celtic_mythology)
  WHERE name = p_card_name;
END;
$$;

-- Now apply all the Celtic meanings
SELECT update_card_celtic_meanings(
  'Nine of Winter',
...
SELECT update_card_celtic_meanings(
  ...
  'clarity, truth, mental power, new ideas, winter insight, clear thought',
  'Represents the Sword of Nuada, one of the four treasures of the Tuatha Dé Danann, brought from the northern city of Findias. No one could escape from it once it was drawn from its sheath, and no one could resist it.'
);

SELECT update_card_celtic_meanings(
  'Four of Winter',
  'Like a warrior resting in the crannog after long battles, taking time to tend wounds and sharpen weapons. The Celts understood the wisdom of strategic retreat and recuperation. In the heart of winter, even the land itself rests beneath snow and ice, gathering strength for the renewal to come. This is sacred rest, protected and necessary, not laziness but preparation for future challenges.',
  ...
  'rest, recuperation, meditation, winter retreat, strategic withdrawal, restoration',
  'Connected to the winter festivals when Celtic communities would retreat indoors, telling stories and preserving knowledge while the land slept beneath snow, gathering strength for the growing season ahead.'
);

SELECT update_card_celtic_meanings(
  'Five of Winter',
  'Like battles fought in winter snow where there are no true victors, only survivors marked by loss. The Celtic tales speak of hollow victories where the cost exceeds the gain. Five warriors remain standing while others have fallen, but there is no celebration in their eyes - only the bitter knowledge of what was sacrificed. This is conflict that leaves everyone diminished, where winning and losing blur into the same grey winter landscape.',
...
SELECT update_card_celtic_meanings(
  ...
  'fairness, balance, law, truth, Brehon justice, cosmic order',
  'Connected to the Brehon legal system and the concept of "fír flathemon" (truth of the ruler) - the belief that a just king would cause the land itself to prosper, while injustice would bring blight and disaster.'
);

SELECT update_card_celtic_meanings(
  'The Hierophant',
  'Like the Druids who held the sacred knowledge and initiated seekers into ancient mysteries, this is the guardian of tradition and spiritual wisdom. The Hierophant stands at the threshold between the mundane and the sacred, teaching those who would learn the old ways. Celtic society revered those who preserved and transmitted sacred knowledge through generations. Seek legitimate spiritual guidance; respect the tested wisdom of tradition while remaining open to genuine understanding.',
  ...
  'spiritual teaching, tradition, sacred knowledge, initiation, Druidic wisdom, guidance',
  'Represents the Druids, the priestly class of Celtic society who spent decades in training, memorizing vast bodies of sacred knowledge, law, poetry, and performing religious ceremonies and initiations.'
);
